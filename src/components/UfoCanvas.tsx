...
export const UfoCanvas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const undersideLightMaterials: { material: THREE.MeshStandardMaterial; phase: number }[] = [];
    ...
    for (let index = 0; index < 28; index += 1) {
      ...
      undersideLightMaterials.push({
        material: bulb.material as THREE.MeshStandardMaterial,
        phase: index * 0.35,
      });
    }
    ufoGroup.add(undersideLights);

    const edgeLightMaterials: { material: THREE.MeshStandardMaterial; phase: number }[] = [];
    ...
    for (let index = 0; index < 18; index += 1) {
      ...
      edgeLightMaterials.push({
        material: edgeLight.material as THREE.MeshStandardMaterial,
        phase: index * 0.52,
      });
    }
    ...
    const beamParticleSeeds: { angle: number; radius: number; speed: number; offset: number }[] = [];
    ...
    stage.addEventListener('pointermove', updatePointerTarget);
    stage.addEventListener('pointerleave', resetPointerTarget);
    ...
    const renderUfoScene = () => {
      ...
      const positions = beamParticleGeometry.attributes.position.array as Float32Array;
      for (let index = 0; index < beamParticleCount; index += 1) {
        const seed = beamParticleSeeds[index];
        const progress = (elapsed * seed.speed + seed.offset) % 1;
        positions[index * 3] = Math.cos(seed.angle + elapsed * 0.18) * seed.radius;
        positions[index * 3 + 1] = -1.65 - progress * 6.7;
        positions[index * 3 + 2] = Math.sin(seed.angle + elapsed * 0.18) * seed.radius;
      }
      beamParticleGeometry.attributes.position.needsUpdate = true;
      beamParticles.rotation.y = elapsed * 0.18;

      renderer.render(scene, camera);
      animationFrameId = window.requestAnimationFrame(renderUfoScene);
    };
    ...
    return () => {
      ...
      stage.removeEventListener('pointermove', updatePointerTarget);
      stage.removeEventListener('pointerleave', resetPointerTarget);
      if (resizeObserver) {
        resizeObserver.disconnect();
      } else {
        window.removeEventListener('resize', resizeHeroUfoScene);
      }
      renderer.dispose();
    };
  }, []);
...
const StyledUfoStage = styled.div`
  position: relative;
  width: min(100%, 620px);
  ...
  .hero-ufo-canvas {
    ...
    position: relative;
    z-index: 1;
    filter: drop-shadow(0 0 38px rgba(87, 220, 255, 0.16));
  }
  ...
  .hero-ufo-fallback {
    ...
    filter: drop-shadow(0 0 42px rgba(87, 220, 255, 0.28));
    transition: opacity 220ms ease;
  }

  &.is-ready .hero-ufo-fallback {
    opacity: 0;
    visibility: hidden;
  }
  ...
  .hero-ufo-aura-two {
    ...
    right: 4%;
    bottom: 1rem;
    background: rgba(87, 220, 255, 0.09);
  }
